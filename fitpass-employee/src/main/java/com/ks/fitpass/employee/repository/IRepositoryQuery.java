...
public interface IRepositoryQuery {
    ...
    String GET_TOTAL_LIST_NEED_CHECK_IN_FLEXIBLE_BY_DEPARTMENT_ID = """
            SELECT COUNT(*)
            FROM order_plan_detail
            WHERE NOW() < plan_expired_time
              AND gym_department_id = ?
              AND price_per_hours > 0
              AND use_status = 'Chưa tập'
            """;
    ...
    String GET_TOTAL_LIST_NEED_CHECK_OUT_FLEXIBLE_BY_DEPARTMENT_ID = """
            SELECT COUNT(*)
            FROM order_plan_detail
            WHERE NOW() < plan_expired_time
                AND gym_department_id = ?
