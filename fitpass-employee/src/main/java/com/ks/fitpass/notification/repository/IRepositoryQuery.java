...
public interface IRepositoryQuery {
    ...
    String GET_CONFIRM_CHECK_IN_BY_USER_RECEIVE_ID = """
            SELECT notification_id, user_id_send, message, time_send, department_id, message_type
            FROM notification
    ...
    String GET_CONFIRM_CHECK_IN_BY_EMP_RECEIVE_ID = """
            SELECT notification_id, user_id_send, message, time_send, department_id, message_type
            FROM notification
            WHERE user_id_receive = ?
            AND time_send = (
            ...
            AND message_type = 'Thông báo xác nhận checkin tới employee'
            AND status = 0;
                """;

    String GET_CONFIRM_CHECK_OUT_BY_EMP_RECEIVE_ID = """
            SELECT notification_id, user_id_send, message, time_send, department_id, message_type
            FROM notification
            WHERE user_id_receive = ?
            AND time_send = (
            ...
            AND message_type = 'Thông báo xác nhận checkout tới employee'
            AND status = 0;
                """;
